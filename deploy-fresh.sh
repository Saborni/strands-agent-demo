...
# Step 1: Create all resources except bedrock runtime
echo "📦 Creating infrastructure (excluding bedrock runtime)..."
cd agent-infra
terraform init
terraform apply -target=aws_ecr_repository.agent_repo \
                -target=aws_ecr_lifecycle_policy.agent_repo_policy \
                -target=aws_iam_role.this \
                -target=aws_iam_role_policy.this \
                -target=aws_cloudwatch_log_group.agentcore_runtime_logs \
                -auto-approve
